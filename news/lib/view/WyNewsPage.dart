...
import 'package:mystocks/model/wynews_enity.dart';
...
class WyNewsPageState extends State<WyNewsPage> {
  ...
  void onItemClick(int i, String articleTitle) {
    String h5_url;
    if ((listData[i].data as BBM54PGAwangning).url == null) {
        h5_url = (listData[i].data as BBM54PGAwangning).skipURL;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
    }else{
       h5_url = (listData[i].data as BBM54PGAwangning).url;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
