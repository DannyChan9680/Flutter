...
import 'package:mystocks/model/military_enity.dart';
...
class MilitaryPage extends StatefulWidget {
  MilitaryPage({Key key}) : super(key: key);

  @override
  MilitaryPageState createState() => new MilitaryPageState();
}

class MilitaryPageState extends State<MilitaryPage> {
...
//    print("加载列表getRow==》" + i.toString());
    BAI67OGGwangning data = listData[i].data;
  ...
  void getDatas(int request_type) async {
    ...
    String url =
        "https://3g.163.com/touch/reconstruct/article/list/BAI67OGGwangning/" +
   ...
   */
    ...
    String h5_url;
    if ((listData[i].data as BAI67OGGwangning).url == null) {
        h5_url = (listData[i].data as BAI67OGGwangning).skipURL;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
    }else{
       h5_url = (listData[i].data as BAI67OGGwangning).url;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
  ...
  void DealDatas(jsonString, int request_type) {
    try {
      var news = new military_enity.fromJson(jsonString);
      if (mounted) {
        setState(() {
          if (request_type == REFRESH_REQIEST) {
            listData.clear();
            for (BAI67OGGwangning data in news.bAI67OGGwangning) {
