...
import 'package:mystocks/model/education_enity.dart';
...
class EducationPage extends StatefulWidget {
  ...
  @override
  EducationPageState createState() => new EducationPageState();
}
...
class EducationPageState extends State<EducationPage> {
  ...
  Widget getItem(int i) {
    BA8FF5PRwangning data = listData[i].data;
  ...
  void getDatas(int request_type) async {
    ...
    String url =
        "https://3g.163.com/touch/reconstruct/article/list/BA8FF5PRwangning/" +
   ...
   */
    ...
    String h5_url;
    if ((listData[i].data as BA8FF5PRwangning).url == null) {
        h5_url = (listData[i].data as BA8FF5PRwangning).skipURL;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
    }else{
       h5_url = (listData[i].data as BA8FF5PRwangning).url;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
  ...
  void DealDatas(jsonString, int request_type) {
    try {
      var news = new education_enity.fromJson(jsonString);
      if (mounted) {
        setState(() {
          if (request_type == REFRESH_REQIEST) {
            listData.clear();
            for (BA8FF5PRwangning data in news.bA8FF5PRwangning) {
