...
import 'package:mystocks/model/finance_enity.dart';
...
class FinancePage extends StatefulWidget {
  ...
  @override
  FinancePageState createState() => new FinancePageState();
}
...
class FinancePageState extends State<FinancePage> {
  ...
  Widget getItem(int i) {
    BA8EE5GMwangning data = listData[i].data;
  ...
  void getDatas(int request_type) async {
    ...
    String url =
        "https://3g.163.com/touch/reconstruct/article/list/BA8EE5GMwangning/" +
   ...
   */
    ...
    String h5_url;
    if ((listData[i].data as BA8EE5GMwangning).url == null) {
        h5_url = (listData[i].data as BA8EE5GMwangning).skipURL;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
    }else{
       h5_url = (listData[i].data as BA8EE5GMwangning).url;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
  ...
  void DealDatas(jsonString, int request_type) {
    try {
      var news = new finance_enity.fromJson(jsonString);
      if (mounted) {
        setState(() {
          if (request_type == REFRESH_REQIEST) {
            listData.clear();
            for (BA8EE5GMwangning data in news.bA8EE5GMwangning) {
