...
import 'package:mystocks/model/science_enity.dart';
...
class SciencePage extends StatefulWidget {
  ...
  @override
  SciencePageState createState() => new SciencePageState();
}

class SciencePageState extends State<SciencePage> {
  ...
  Widget getItem(int i) {
    BA8D4A3Rwangning data = listData[i].data;
  ...
  void getDatas(int request_type) async {
    ...
    String url =
        "https://3g.163.com/touch/reconstruct/article/list/BA8D4A3Rwangning/" +
   ...
   */
    ...
    String h5_url;
    if ((listData[i].data as BA8D4A3Rwangning).url == null) {
        h5_url = (listData[i].data as BA8D4A3Rwangning).skipURL;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
    }else{
       h5_url = (listData[i].data as BA8D4A3Rwangning).url;
    Navigator.push(
        context,
        new MaterialPageRoute(
            builder: (context) => new NewsWebPage(h5_url, '新闻详情')));
  ...
  void DealDatas(jsonString, int request_type) {
    try {
      var news = new science_enity.fromJson(jsonString);
      if (mounted) {
        setState(() {
          if (request_type == REFRESH_REQIEST) {
            listData.clear();
            for (BA8D4A3Rwangning data in news.bA8D4A3Rwangning) {
